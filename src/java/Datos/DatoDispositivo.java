...
//    Numero de Tarjeta, Periodo de Cobro, Numero de Factura
    public List<SelectItem> getTarjetaNumero() {
        List<SelectItem> numeroTarjeta = new ArrayList<SelectItem>();
        ResultSet rs;
        try {
            this.Conectar();
//        per.setFechaRegisto(this.obtenerHora());
            PreparedStatement st = this.getCn().prepareStatement("SELECT tarjeta_id FROM tarjeta");
            rs = st.executeQuery();

            while (rs.next()) {
                numeroTarjeta.add(new SelectItem(rs.getString("tarjeta_id")));
            }
    ...
    public List<SelectItem> getPeriodoCobro() {
        List<SelectItem> periodoCobro = new ArrayList<SelectItem>();
        Dispositivo dispositivo = new Dispositivo();
        ResultSet rs;
        try {
            this.Conectar();
//        per.setFechaRegisto(this.obtenerHora());
            PreparedStatement st = this.getCn().prepareStatement("SELECT dispositivo_periodoCobro from dispositivo where dispositivo_id=(select items_id from items where items_name='" + dispositivo.getFechaCompra() + "')");
            rs = st.executeQuery();

            while (rs.next()) {
                periodoCobro.add(new SelectItem(rs.getString("dispositivo_periodoCobro")));
            }
    ...
    public List<SelectItem> getFacturaNumero() {
        ...
        try {
            this.Conectar();
//        per.setFechaRegisto(this.obtenerHora());
            PreparedStatement st = this.getCn().prepareStatement("SELECT dispositivo_factura from dispositivo where dispositivo_id=(select items_id from items where items_name='" + dispositivo.getPlacaVehiculo() + "')");
            rs = st.executeQuery();

            while (rs.next()) {
                peajeSentido.add(new SelectItem(rs.getString("dispositivo_factura")));
            }
    ...
    public List<SelectItem> getMontoFactura() {
        List<SelectItem> montoFactura = new ArrayList<SelectItem>();
        ResultSet rs;
        try {
            this.Conectar();
//        per.setFechaRegisto(this.obtenerHora());
            PreparedStatement st = this.getCn().prepareStatement("SELECT dispositivo_montoFacturas FROM tarjeta");
            rs = st.executeQuery();

            while (rs.next()) {
                montoFactura.add(new SelectItem(rs.getString("dispositivo_montoFacturas")));
            }
